...
/* CmuVolunteer.sol
   Modified from MyAdvancedToken.sol
*/
...
contract TokenERC20 {
    ...
    */
    ...
    // This generates a public event on the blockchain that can be
    // used to notify the clients.
    event TransferVoucher(address indexed from, address indexed to, uint256 value);
    ...
    // This can be used to notify clients about certificate added.
    event Certificate(address indexed from, bytes32 value, uint256 hrs);
...
contract CMUCoin is owned, TokenERC20 {
    ...
    */
    ...
    /*
      An internal method to convert cmu coins to a voucher
    */
    function _getVoucher(address _from, address _to, uint _value) internal {

        // Check if the value sent is enough
        require(_value >= 20000000000000000000);

        // Make no. of tokens to be a whole number
        if (_value % 20 != 0) {
            _value /= 20;
            _value *= 20;
        }
        ...
        // Make notification of the transfer
        emit TransferVoucher(_from, _to, _value/20);
    ...
    /*
      An internal method to add certificate
    */
    function _addCertificate(address _from, bytes32 _value, uint256 _hrs) internal {
        // Make notification of the transfer
        emit Certificate(_from, _value, _hrs);
    ...
    */
    ...
    /* Public transfer of tokens to get voucher.
       Calls the internal transfer with the message sender as 'from'.
   */

    function getVoucher(uint256 _value) public returns (bool success) {
        _getVoucher(msg.sender, address(this), _value);
        return true;
    }
   ...
   */
    ...
    function addCertificate(bytes32 _value, uint256 _hrs) public returns (bool success) {
        _addCertificate(msg.sender, _value, _hrs);
        _transfer(address(this), msg.sender, 1000000000000000000*_hrs);
        return true;
    }

    // ------------------------------------------------------------------------
    // 1,0 CMU Tokens per 1 ETH
    // ------------------------------------------------------------------------
